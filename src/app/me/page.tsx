...
const emptyGroups: ContentGroups = { skills: [], posts: [], comments: [] };
...
export default function MyPage() {
  ...
  useEffect(() => {
    async function loadProfile() {
      ...
      setUser(data.user);
      const [skills, posts, comments] = await Promise.all([
      ...
      const firstError = [skills.error, posts.error, comments.error].find(Boolean);
  ...
  const total = content.skills.length + content.posts.length + content.comments.length;
