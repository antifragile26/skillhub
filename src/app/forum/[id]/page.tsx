...
export default function PostDetailPage() {
  ...
  const [commentVoteBusy, setCommentVoteBusy] = useState<Record<string, boolean>>({});
  ...
  useEffect(() => {
    async function load() {
      ...
      const commentsQuery = await supabase.from("comments").select("id, post_id, user_id, author, content, created_at, updated_at, upvotes, downvotes").eq("post_id", postId).is("deleted_at", null).order("created_at", { ascending: true });
      ...
      if (postResult.data && !postResult.data.deleted_at) {
        const loaded = postResult.data as Post;
        setPost(loaded); setEditTitle(loaded.title); setEditContent(loaded.content ?? ""); setUpvotes(loaded.upvotes ?? 0); setDownvotes(loaded.downvotes ?? 0);
      } else setMessage(postResult.error ? "帖子加载失败，请重试。" : "帖子不存在、已删除或暂时不可见。");
      if (commentsResult.error) setMessage("回复加载失败，请重试。");
      else {
        const loadedComments = (commentsResult.data ?? []) as Comment[];
        setComments(loadedComments);
        if (user && loadedComments.length) {
          const ownVoteQuery = await supabase.from("comment_votes").select("comment_id, vote_type").in("comment_id", loadedComments.map((comment) => comment.id)).eq("user_id", user.id);
          if (!ownVoteQuery.error) {
            const ownVotes: Record<string, "up" | "down" | null> = {};
            for (const vote of ownVoteQuery.data ?? []) ownVotes[vote.comment_id] = vote.vote_type as "up" | "down";
            setCommentVotes(ownVotes);
          }
        }
      }
      setMyVote(voteResult.data?.vote_type === "up" || voteResult.data?.vote_type === "down" ? voteResult.data.vote_type : null);
      setIsLoading(false);
    }
    void load();
  }, [postId, user]);
  ...
  async function changeVote(kind: "up" | "down") {
    ...
    setVoteBusy(true); setMessage("");
    const requestId = makeRequestId(); const limit = await supabase.rpc("check_forum_rate_limit", { p_action: "vote", p_request_id: requestId });
  ...
  async function changeCommentVote(commentId: string, kind: "up" | "down") {
    if (!user || commentVoteBusy[commentId]) { if (!user) router.push(`/login?returnTo=${encodeURIComponent(`/forum/${postId}`)}`); return; }
    setCommentVoteBusy((current) => ({ ...current, [commentId]: true })); setMessage("");
    const requestId = makeRequestId(); const limit = await supabase.rpc("check_forum_rate_limit", { p_action: "vote", p_request_id: requestId });
    if (limit.error || !(limit.data as { allowed?: boolean } | null)?.allowed) { setMessage(limit.error ? "评论投票服务尚未完成数据库升级，请稍后重试。" : "投票过于频繁，请稍后再试。"); setCommentVoteBusy((current) => ({ ...current, [commentId]: false })); return; }
    const currentVote = commentVotes[commentId]; const removing = currentVote === kind; let error;
    if (removing) ({ error } = await supabase.from("comment_votes").delete().eq("comment_id", commentId).eq("user_id", user.id));
    else ({ error } = await supabase.from("comment_votes").upsert({ comment_id: commentId, user_id: user.id, vote_type: kind }, { onConflict: "user_id,comment_id" }));
    if (error) setMessage(`评论投票失败：${error.message}`);
    else {
      setCommentVotes((current) => ({ ...current, [commentId]: removing ? null : kind }));
      const { data } = await supabase.from("comments").select("upvotes, downvotes").eq("id", commentId).maybeSingle();
      if (data) setComments((items) => items.map((item) => item.id === commentId ? { ...item, upvotes: data.upvotes, downvotes: data.downvotes } : item));
    }
  ...
  async function handleReply() {
    ...
    const { data, error } = await supabase.from("comments").upsert({ post_id: postId, user_id: user.id, author: getProfileDisplay(user).name, content: normalizeForumText(replyText), request_id: requestId }, { onConflict: "user_id,request_id" }).select("id, post_id, user_id, author, content, created_at, updated_at, upvotes, downvotes").single();
