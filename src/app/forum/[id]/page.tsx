...
type Post = { id: string; title: string; content?: string | null; category?: string | null; content_type?: string | null; status?: string | null; author?: string | null; created_at?: string | null; upvotes?: number | null; downvotes?: number | null; user_id?: string | null; resolved?: boolean; accepted_comment_id?: string | null; is_featured?: boolean; case_scenario?: string | null; case_goal?: string | null; case_tools_environment?: string | null; case_steps?: string | null; case_input_example?: string | null; case_output_example?: string | null; case_effects?: string | null; case_limitations?: string | null };
...
type KnowledgeEntryLink = { id: string; title: string };
...
export default function PostDetailPage() {
  ...
  const [knowledgeEntries, setKnowledgeEntries] = useState<KnowledgeEntryLink[]>([]);
  ...
  useEffect(() => {
    async function load() {
      ...
      await Promise.all(loadedLinks.map(async (link) => {
        if (link.product_type !== "skill") return;
        const result = await supabase.from("skills").select("id,name").eq("id", link.product_id).maybeSingle();
        if (result.data) nextProducts[link.id] = { name: result.data.name, href: `/skills/${result.data.id}` };
      }));
  ...
  if (loading) return <main className="hub-page"><div className="hub-container py-12 hub-muted">正在加载帖子…</div></main>;
  if (!post) return <main className="hub-page"><div className="hub-container py-12"><Link href="/forum" className="text-sm text-[var(--accent-strong)]">← 返回论坛</Link><p className="mt-8">{message || "帖子不存在或暂时不可见。"}</p></div></main>;
  ...
  return <main className="hub-page"><section className="hub-container max-w-5xl py-8 sm:py-10"><Link href="/forum" className="text-sm font-medium text-[var(--accent-strong)] hover:underline">← 返回论坛</Link>
    <article className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm shadow-slate-900/5 dark:border-slate-700 dark:bg-slate-900/80 dark:shadow-black/20"><div className="border-b border-slate-100 px-6 pb-5 pt-6 dark:border-slate-700/80"><div className="mb-4 flex flex-wrap gap-2"><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-300">{categoryLabel(post.category) || "综合讨论"}</span>{post.content_type === "case" && <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300">经验分享</span>}{post.status === "pending" && <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-medium text-amber-700 dark:bg-amber-950/50 dark:text-amber-300">待审核</span>}{post.resolved && <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-medium text-blue-700 dark:bg-blue-950/50 dark:text-blue-300">已解决</span>}{post.is_featured && <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-medium text-amber-700 dark:bg-amber-950/50 dark:text-amber-300">精选</span>}</div>
      ...
      {post.status === "pending" && isPostOwner && <div className="border-t border-amber-100 bg-amber-50 px-6 py-3 text-sm text-amber-800 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-200">这篇帖子已提交审核，审核通过后会出现在作品详情和公开论坛中。</div>}
    ...
    {knowledgeEntries.length > 0 && <section className="mt-6 rounded-2xl border border-blue-200 bg-blue-50/60 p-5 dark:border-blue-900/60 dark:bg-blue-950/20"><h2 className="font-bold">已沉淀为知识</h2><div className="mt-3 flex flex-wrap gap-2">{knowledgeEntries.map((entry) => <Link key={entry.id} href={`/knowledge/${entry.id}`} className="rounded-lg border border-blue-200 bg-white px-3 py-2 text-sm text-blue-700 hover:border-blue-400 hover:underline dark:border-blue-900 dark:bg-slate-900 dark:text-blue-300">{entry.title} →</Link>)}</div></section>}
    {links.length > 0 && <section className="mt-6"><h2 className="mb-3 text-lg font-bold">讨论的 Skill</h2><div className="grid gap-3 sm:grid-cols-2">{links.map((link) => link.product_type === "agent" ? <div key={link.id} className="rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-500 dark:border-slate-700">关联的 Agent 已停止展示</div> : products[link.id] ? <Link key={link.id} href={products[link.id].href} onClick={() => void clickProduct(link)} className="rounded-xl border border-slate-200 bg-white p-4 transition hover:border-blue-400 hover:shadow-sm dark:border-slate-700 dark:bg-slate-900/70"><span className="text-xs text-slate-500">Skill</span><p className="mt-1 font-medium">{products[link.id].name}</p><span className="mt-2 inline-block text-sm font-medium text-blue-600 dark:text-blue-300">返回 Skill 详情 →</span></Link> : <div key={link.id} className="rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-500">关联的 Skill 当前不可访问</div>)}</div></section>}
