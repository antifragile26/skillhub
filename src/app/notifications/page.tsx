...
export default function NotificationsPage() {
  ...
  return <main className="min-h-screen bg-white px-5 py-10 text-zinc-900 dark:bg-[#0a0e14] dark:text-zinc-100 sm:px-8"><div className="mx-auto max-w-3xl"><div className="flex items-center justify-between"><div><Link href="/forum" className="text-sm text-blue-600">← 返回论坛</Link><h1 className="mt-4 text-3xl font-bold">通知</h1></div><button type="button" onClick={() => void mark()} className="text-sm text-blue-600">全部标为已读</button></div>{message && <p className="mt-8 rounded-md bg-amber-50 p-4 text-sm text-amber-800">{message}</p>}<div className="mt-8 space-y-3">{notices.length === 0 && !message ? <p className="py-10 text-center text-sm text-zinc-500">暂无通知。</p> : notices.map((notice) => <article key={notice.id} className={`rounded-lg border p-4 ${notice.read_at ? "border-zinc-200 dark:border-zinc-800" : "border-blue-300 bg-blue-50/50 dark:border-blue-900 dark:bg-blue-950/20"}`}><div className="flex items-start justify-between gap-4"><p className="text-sm">{notice.message}</p>{!notice.read_at && <button type="button" onClick={() => void mark(notice.id)} className="shrink-0 text-xs text-blue-600">标为已读</button>}</div><div className="mt-2 flex gap-3 text-xs text-zinc-500"><time>{new Date(notice.created_at).toLocaleString("zh-CN")}</time>{notice.target_type === "post" && notice.target_id && <Link href={`/forum/${notice.target_id}`} className="text-blue-600">查看帖子</Link>}</div></article>)}</div></div></main>;
}
