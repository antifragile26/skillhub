...
2026-09-19 更新：按用户澄清移除后台“生成演示数据”按钮；匿名只读检查线上 `demo_batch2_` 帖子为 0。只基于现有真实帖子准备了来源可追溯、明确标注待核验的知识条目，以及发给已确认管理员的两条真实站内提醒。迁移 `20260919010000_curated_real_knowledge.sql` 尚待 Supabase SQL Editor 执行；不将虚构内容或违规指控写入生产。
